...
func Map(value string) *list.List {
	...
	for word, count := range wcMap {
		kvMapList.PushBack(mapreduce.KeyValue{word, strconv.Itoa(count)})
	}
...
func Reduce(key string, values *list.List) string {
	...
	for e := values.Front(); e != nil; e = e.Next() {
		countStr, ok := e.Value.(string)
		if !ok {
			log.Fatal("get count FAILED")
		} else {
			count, err := strconv.Atoi(countStr)
			if err != nil {
				log.Fatal("strconv.Atoi(countStr) for " + countStr)
			}
			wc += count
		}
	}
...
func main() {
	if len(os.Args) != 4 {
		...
		if os.Args[3] == "sequential" {
			...
			mr := mapreduce.MakeMapReduce(1, 1, os.Args[2], os.Args[3])
